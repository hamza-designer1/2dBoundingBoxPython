...
def project_point_to_image(point, sensorLocation, sensorRotation):
    ...
    """
    # Directly transform the point from world to sensor (camera) coordinates
    sensor_point = transform_point(point, sensorLocation, sensorRotation, inverse=True)
...
for filename_pos, filename_loc, filename_rot in zip(file_names_pos, file_names_loc, file_names_rot):
    ...
    for pose in posses:
        # Example pose coordinates (in meters)
        pose_values = (6.0, 2.0, 3.0)


        # Example camera location and rotation (in meters and degrees)
